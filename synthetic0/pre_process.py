...
def main():
    ...
    arff_data = arff_input.loadarff('original-files/SynHyperPlane_drift_novelty_train.arff')
    arff_df = pd.DataFrame(arff_data[0])
    df = pd.DataFrame(arff_df.values)
    df = df.apply(pd.to_numeric)

    # Splitting data and labels
    # Note: the labels are stored in the first columns, instead of the last
    data = df.iloc[:, CLASS_COUNT:].reset_index(drop=True)
    labels = df.iloc[:, :CLASS_COUNT].reset_index(drop=True)
